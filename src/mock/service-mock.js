...
export default class MockService {
  ...
  constructor () {
    this.destination = generateDestinationObj();
  ...
  generatePoints() {
    return Array.from({length: POINTS_COUNT}, () => {
      ...
      const destination = getRandomArrayElement(this.generateDestinations());

      const hasOffers = getRandomInteger(0, 1);

      const offersByType = this.generateOffers().find((offerByType) => offerByType.type === type);
